...
beforeEach(() => {
  _resetMemoryStore();
  authMock.mockReset().mockResolvedValue({ userId: "clerk-p1", isAuthenticated: true });
...
describe("POST /api/races/[id]/draw — offer", () => {
  ...
  it("returns 403 not_participant for an outsider", async () => {
    requireLinkedUserMock.mockResolvedValue({
      ok: true,
      user: { id: OUTSIDER, clerkId: "clerk-out", username: "out", cfHandle: null, cfRating: null, cfLinkedAt: new Date(), elo: 1200, racesPlayed: 0, cppTemplate: "", solveHistorySyncedAt: null, solveHistoryImportCursor: null, createdAt: null, isAdmin: false },
    });
...
describe("POST /api/races/[id]/draw — accept", () => {
  it("atomically consumes the opponent's offer, finishes as a draw, returns the fresh snapshot", async () => {
    ...
    requireLinkedUserMock.mockResolvedValue({
      ok: true,
      user: { id: P2, clerkId: "clerk-p2", username: "p2", cfHandle: "p2cf", cfRating: 1400, cfLinkedAt: new Date(), elo: 1200, racesPlayed: 3, cppTemplate: "", solveHistorySyncedAt: null, solveHistoryImportCursor: null, createdAt: null, isAdmin: false },
    });
