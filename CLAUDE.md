...
- **Migrations auto-apply on deploy**: `vercel-build` runs `drizzle-kit migrate && next build`, so merging a migration to main migrates prod. **There is only ONE database** — `.env.local`'s `DATABASE_URL` is the same Neon DB production uses. Additive migrations may be pre-applied locally (`pnpm exec drizzle-kit migrate`); destructive migrations (DROP COLUMN/TABLE) must NOT be run locally while the deployed code still references the schema — let the deploy apply them, or expand-then-contract.
