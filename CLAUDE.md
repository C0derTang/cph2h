...
- **CF has no submit API** — submission replays the website login/submit flow (`src/lib/cf/auth.ts`, `submit.ts`); verdicts come from polling `user.status`. CF passwords + session cookies are encrypted at rest (AES-256-GCM, `src/lib/crypto.ts`). Never log credentials or source.
